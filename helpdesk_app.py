...
try:
    ...
    HAS_PDF = True
    logger.info("✅ ReportLab successfully imported - PDF generation available")
except Exception as e:
    HAS_PDF = False
    logger.warning(f"⚠️ ReportLab not available - PDF generation disabled: {str(e)}")
...
def generate_asset_signout_pdf(asset_data, recipient_name, recipient_email, recipient_phone, signature, signout_date):
    """Generate PDF for asset sign-out form"""
    if not HAS_PDF:
        return None
    ...
    title_style = ParagraphStyle(
        ...
        fontSize=24,
        textColor=colors.HexColor(VDH_NAVY),
        spaceAfter=30,
        alignment=1  # Center
    )
    ...
    # Title
    elements.append(Paragraph("ASSET SIGN-OUT FORM", title_style))
    elements.append(Paragraph("Virginia Department of Health", styles['Normal']))
    elements.append(Paragraph("Crater Service Center", styles['Normal']))
    elements.append(Spacer(1, 0.3*inch))
    
    # Date and Document Info
    from datetime import datetime
    elements.append(Paragraph(f"<b>Date:</b> {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", styles['Normal']))
    elements.append(Paragraph(f"<b>Sign-Out Date:</b> {signout_date}", styles['Normal']))
    ...
    asset_data_table = [
        ['Asset Tag:', asset_data.get('asset_tag', 'N/A')],
        ['Asset Type:', asset_data.get('type', 'N/A')],
        ['Category:', asset_data.get('category', 'N/A')],
        ['Make/Model:', asset_data.get('model', 'N/A')],
        ['Serial Number:', asset_data.get('serial', 'N/A')],
        ['Location:', asset_data.get('location', 'N/A')],
    ]
    
    asset_table = Table(asset_data_table, colWidths=[2*inch, 4*inch])
    asset_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (0, -1), colors.HexColor('#E8E8E8')),
        ('TEXTCOLOR', (0, 0), (0, -1), colors.HexColor(VDH_NAVY)),
    ...
    recipient_data_table = [
        ['Full Name:', recipient_name],
        ['Email:', recipient_email],
        ['Phone:', recipient_phone],
    ]
    
    recipient_table = Table(recipient_data_table, colWidths=[2*inch, 4*inch])
    ...
    signature_table_data = [[Paragraph(signature, sig_style)]]
    signature_table = Table(signature_table_data, colWidths=[6*inch], rowHeights=[1*inch])
    signature_table.setStyle(TableStyle([
        ('BOX', (0, 0), (-1, -1), 2, colors.HexColor(VDH_NAVY)),
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ...
    elements.append(Paragraph(f"<i>Electronically signed on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}</i>", styles['Normal']))
    elements.append(Spacer(1, 0.3*inch))
    
    # Acknowledgment Section
    elements.append(Paragraph("ACKNOWLEDGMENT", heading_style))
    acknowledgment_text = """
    By signing this form, I acknowledge that I have received the above-listed asset in good working condition. 
    I understand that I am responsible for the proper care, use, and security of this equipment. 
    I agree to return this asset upon request or upon separation from the Virginia Department of Health.
    """
    elements.append(Paragraph(acknowledgment_text, styles['Normal']))
    elements.append(Spacer(1, 0.3*inch))
    ...
    elements.append(Spacer(1, 0.3*inch))
    elements.append(Paragraph("___" * 60, footer_style))
    elements.append(Paragraph("Virginia Department of Health - Crater Service Center", footer_style))
    elements.append(Paragraph("For official use only. Keep this form with asset records.", footer_style))
...
def render_distribution_platform():
    ...
    st.subheader("📊 Resource Distribution Platform")
...
def render_resource_management():
    ...
    with col5:
        if st.button("📊 Distribution", use_container_width=True,
                    type="primary" if st.session_state.resource_view == 'distribution' else "secondary"):
    ...
    elif st.session_state.resource_view == 'distribution':
        render_distribution_platform()
...
def main():
    ...
    # CONNECTED MODE: Helpdesk Tickets - attempt to list tickets when DB_AVAILABLE is True
        ...
        # CREATE TICKET BUTTON (only show when not viewing/editing/creating)
        if not st.session_state.quick_create_ticket and not st.session_state.view_ticket_id and not st.session_state.edit_ticket_id:
            if st.button("➕ Create New Ticket", type="primary", key="create_ticket_page_btn"):
        ...
        if 'sign_out_asset_id' not in st.session_state:
            st.session_state.sign_out_asset_id = None
        if 'signout_complete' not in st.session_state:
            st.session_state.signout_complete = False
        
        # ADD ASSET BUTTON (only show when not viewing/editing/adding/signing out/showing PDF)
        if DB_AVAILABLE and not st.session_state.view_asset_id and not st.session_state.edit_asset_id and not st.session_state.add_new_asset and not st.session_state.sign_out_asset_id and not st.session_state.get('signout_complete', False):
        ...
        if not DB_AVAILABLE:
            ...
            if st.session_state.view_asset_id:
                ...
                with col4:
                    if st.button("✍️ Sign Out"):
                        st.session_state.sign_out_asset_id = st.session_state.view_asset_id
                        st.session_state.view_asset_id = None
                        st.rerun()
            ...
            # ASSET SIGN-OUT FORM
            elif st.session_state.sign_out_asset_id:
                st.markdown("---")
                col1, col2 = st.columns([1, 4])
                with col1:
                    if st.button("← Back"):
                ...
                # Fetch asset data
                asset_query = f"SELECT * FROM dbo.Assets WHERE asset_id = {st.session_state.sign_out_asset_id}"
                ...
                if asset_err or asset_df is None or len(asset_df) == 0:
                    ...
                    st.subheader(f"✍️ Sign Out Asset: {asset.get('asset_tag', 'N/A')}")
                    st.info("📋 Complete the form below to assign this asset to a user")
                    ...
                    with st.expander("📦 Asset Details", expanded=False):
                        ...
                        with col2:
                            st.write(f"**Serial:** {asset.get('serial', 'N/A')}")
                            st.write(f"**Current Status:** {asset.get('status', 'N/A')}")
                            st.write(f"**Location:** {asset.get('location', 'N/A')}")
                    
                    st.markdown("---")
                    
                    with st.form("sign_out_asset_form"):
                        st.markdown("### 👤 Recipient Information")
                        
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            recipient_first_name = st.text_input("First Name *", key="signout_first")
                            recipient_last_name = st.text_input("Last Name *", key="signout_last")
                            recipient_email = st.text_input("Email *", key="signout_email")
                        
                        with col2:
                            recipient_phone = st.text_input("Phone Number *", key="signout_phone")
                            signout_date = st.date_input("Sign Out Date", value=datetime.now().date())
                            st.write("")  # Spacing
                        
                        st.markdown("---")
                        st.markdown("### ✍️ Digital Signature")
                        st.caption("By signing below, the recipient acknowledges receipt of this asset and accepts responsibility for its care and return.")
                        
                        # Signature method selection
                        sig_method = st.radio(
                            "Signature Method", 
                            ["✍️ Type Signature", "🎨 Draw Signature (Coming Soon)"], 
                            horizontal=True,
                            key="sig_method_asset"
                        )
                                ...
                                st.markdown(f"""
                                    ...
                                    ">
                                        ...
                                        <p style="
                                            ...
                                            text-align: center;
                                        ">
                                            Signed on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}
                                        </p>
                                    </div>
                                """, unsafe_allow_html=True)
                        ...
                        acknowledge = st.checkbox(
                            "I acknowledge that I have received the above asset in good working condition and will return it upon request.",
                            key="acknowledge_asset"
                        )
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            submit = st.form_submit_button("✅ Complete Sign Out", type="primary", use_container_width=True)
                        with col2:
                            cancel = st.form_submit_button("❌ Cancel", use_container_width=True)
                        ...
                        if submit:
                            # Validation
                            if not recipient_first_name or not recipient_last_name or not recipient_email or not recipient_phone:
                                st.error("❌ Please fill in all required recipient information fields")
                            elif not signature_data:
                                st.error("❌ Please provide a signature")
                            elif not acknowledge:
                                st.error("❌ Please check the acknowledgment box")
                            else:
                                try:
                                    # Build full name
                                    recipient_name = f"{recipient_first_name} {recipient_last_name}".strip()
                                    
                                    # Update asset with recipient info and change status to Deployed
                                    update_query = """
                                        UPDATE dbo.Assets
                                        SET assigned_user = ?,
                                            assigned_email = ?,
                                    ...
                                    cursor.execute(
                                        ...
                                        st.session_state.sign_out_asset_id
                                    )
                                    
                                    # Log the sign-out in Asset_Notes table (if it exists)
                                    try:
                                        note_query = """
                                            INSERT INTO dbo.Asset_Notes (asset_id, note_text, note_type, created_by, created_at)
                                            VALUES (?, ?, ?, ?, GETDATE())
                                        """
                                        note_text = f"Asset signed out to {recipient_name}. Signature: {signature_data}. Date: {signout_date}"
                                        cursor.execute(note_query, 
                                                     (st.session_state.sign_out_asset_id, note_text, 'Sign Out', 
                                                      st.session_state.get('username', 'System')))
                                    except:
                                        pass  # Asset_Notes table might not exist
                                    ...
                                    try:
                                        ...
                                        pdf_buffer = generate_asset_signout_pdf(
                                            asset_data=asset.to_dict(),
                                            recipient_name=recipient_name,
                                            recipient_email=recipient_email,
                                            recipient_phone=recipient_phone,
                                            signature=signature_data,
                                            signout_date=str(signout_date)
                                        )
                                        
                                        if pdf_buffer:
                                            # Store PDF in session state
                                            filename = f"Asset_SignOut_{asset.get('asset_tag')}_{recipient_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.pdf"
                                            st.session_state.signout_pdf = pdf_buffer.getvalue()
                                            st.session_state.signout_pdf_filename = filename
                                            st.session_state.signout_asset_tag = asset.get('asset_tag')
                                            st.session_state.signout_recipient = recipient_name
                                            logger.info("✅ PDF generated successfully")
                                        else:
                                            st.session_state.signout_pdf = None
                                            logger.warning("⚠️ PDF generation returned None - reportlab may not be available")
                                            st.warning("⚠️ PDF generation skipped (reportlab not available)")
                                    except Exception as pdf_error:
                                        st.session_state.signout_pdf = None
                                        logger.error(f"❌ PDF generation error: {str(pdf_error)}", exc_info=True)
                                        st.error(f"⚠️ PDF generation failed: {str(pdf_error)}")
                                        st.info("💡 The asset was signed out successfully. To enable PDF generation, ensure reportlab is installed.")
                                    
                                    # Mark sign-out as complete
                                    st.session_state.signout_complete = True
                                    st.session_state.sign_out_asset_id = None
                                    st.rerun()
                                    
                                except Exception as e:
                                    st.error(f"❌ Error signing out asset: {str(e)}")
            
            # Show PDF download screen if sign-out just completed
            if st.session_state.get('signout_complete', False):
                st.markdown("---")
                st.success(f"✅ Asset '{st.session_state.get('signout_asset_tag')}' successfully signed out to {st.session_state.get('signout_recipient')}!")
                st.success("📋 Status changed to 'Deployed'")
                
                if st.session_state.get('signout_pdf'):
                    st.success("📄 Sign-out form generated!")
                    
                    # Prominent download button
                    st.markdown("### 📥 Download Sign-Out Form")
                    st.download_button(
                        label="📥 Click Here to Download PDF",
                        data=st.session_state.signout_pdf,
                        file_name=st.session_state.signout_pdf_filename,
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("← Return to Asset List", type="primary", use_container_width=True):
                        # Clear session state
                        st.session_state.signout_complete = False
                        st.session_state.signout_pdf = None
                        st.session_state.signout_pdf_filename = None
                        st.session_state.signout_asset_tag = None
                        st.session_state.signout_recipient = None
                        st.rerun()
                with col2:
                    if st.session_state.get('signout_pdf'):
                        # Duplicate download button for convenience
                        st.download_button(
                            ...
                            mime="application/pdf",
                            use_container_width=True,
                            key="pdf_download_duplicate"
                        )
                
                return  # Exit early so we don't show asset list
    ...
    # Report Builder and Connection Test
        ...
        # This section is now handled within Resource Management
